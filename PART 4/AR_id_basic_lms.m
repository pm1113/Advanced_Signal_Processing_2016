...
[y_hat, error, est_coeff_mat] = lms_ar(y, mu, order);
...
% graphing settings
str = sprintf('\fontsize{40}Identification of AR(%d) process using Basic LMS Algorithm: \\mu = %d', length(a) - 1, mu);
...
str = sprintf('Evolution of Coefficients using Basic LMS Algorithm: \\mu = %d', mu);
title(str, 'FontSize', 40);
xlabel('\fontsize{35}Sample Number');
...
str = sprintf('Squared Error with Basic LMS Algorithm: \\mu = %.4f', mu);
title(str, 'FontSize', 40);
xlabel('\fontsize{35}Sample number');
